...
===== `with`
A map of the input parameters defined by the action in its `action.yml` file. When the action is container based, special parameter names are:
...
==== Job `strategy`
A build matrix strategy is a set of different configurations of the virtual environment. The job’ set of steps will be executed on each of these configurations. The following example specifies 3 nodejs versions on 2 operating systems:
